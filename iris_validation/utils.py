...
CHI_ATOMS = [
    ...
    {("CG", "CD", "NE", "CZ"): ("ARG"), ("CG", "CD", "CE", "NZ"): ("LYS")},
    {("CD", "NE", "CZ", "NH1"): ("ARG")},
]
...
def code_type(mmol_residue):
    try:
        return next(
            category
            for category, group in THREE_LETTER_CODES.items()
            if mmol_residue.type().trim() in group
        )
...
def get_backbone_atoms(mmol_residue):
    try:
        n = next(
            ...
            if atom.id().trim().replace(" ", "") == "N"
            or atom.id().trim().replace(" ", "") == "N:A"
        )
    except StopIteration:
        n = None
    try:
        ca = next(
            atom
            for atom in mmol_residue
            if atom.id().trim().replace(" ", "") == "CA"
            or atom.id().trim().replace(" ", "") == "CA:A"
        )
    except StopIteration:
        ca = None
    try:
        c = next(
            atom
            for atom in mmol_residue
            if atom.id().trim().replace(" ", "") == "C"
            or atom.id().trim().replace(" ", "") == "C:A"
        )
    except StopIteration:
        c = None
    return n, ca, c
...
def calculate_chis(mmol_residue):
    ...
    for i in range(5):
        chi_atoms = []
        has_chi = any(
            mmol_residue.type().trim() in residues
            for residues in list(CHI_ATOMS[i].values())
        )
        if not has_chi:
            return chis
        required_atom_names = next(
            atoms
            for atoms, residues in CHI_ATOMS[i].items()
            if mmol_residue.type().trim() in residues
        )
        missing_atom_names = []
        ...
        if len(chi_atoms) < 4:
            ...
            continue
        xyzs = [
            (atom.coord_orth().x(), atom.coord_orth().y(), atom.coord_orth().z())
...
def analyse_b_factors(mmol_residue, is_aa=None, backbone_atoms=None):
    ...
    b_stdev = (
        sum([(x - b_avg) ** 2 for x in residue_b_factors]) / len(residue_b_factors)
    ) ** 0.5
...
def check_is_aa(mmol_residue, strict=False):
    ...
    if (
        code_type(mmol_residue) in allowed_types
        and None not in get_backbone_atoms(mmol_residue)
        and check_backbone_geometry(mmol_residue)
    ):
        return True
    return False
...
def get_ramachandran_allowed(
    mmol_residue, code=None, phi=None, psi=None, thresholds=None
):
...
def get_ramachandran_classification(
    mmol_residue, code=None, phi=None, psi=None, thresholds=None
):
    if phi is None or psi is None:
        return None
