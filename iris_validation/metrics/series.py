...
class MetricsModelSeries:
    ...
    def align_models(self):
        ...
        if len(self.metrics_models) > 2:
            raise Exception(
                "Iris currently only supports alignment for two model iterations"
            )
        ...
        if len(bad_chain_ids) > 0:
            print(
                "WARNING: at least one chain contains no amino acid residues. Ignoring chains: "
                + ", ".join(sorted(bad_chain_ids))
            )
            for model in self.metrics_models:
        ...
        # Align chains
        chain_id_sets = [
            set(chain.chain_id for chain in model) for model in self.metrics_models
        ]
        ...
        if len(lost_chain_ids) > 0:
            print(
                f"WARNING: Some chains are not present or valid across all model versions ({sorted(lost_chain_ids)}). These chains will not be represented in the validation report."
            )
        ...
        for chain_id in sorted(common_chain_ids):
            ...
            for model in self.metrics_models:
                matching_chain = [
                    chain for chain in model if chain.chain_id == chain_id
                ][0]
        ...
        for chain_id, chain_set in self.chain_sets.items():
            sequences = [
                utils.code_three_to_one([residue.code for residue in chain])
    ...
    def get_raw_data(self):
        ...
        for chain_id, chain_set in self.chain_sets.items():
            ...
            chain_data["percentile_values"] = list(
                zip(*chain_data["percentile_values"])
            )
