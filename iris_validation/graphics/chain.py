...
class ChainView:
    ...
    def _draw(self):
        ...
        # Set HTML attributes
        self.dwg.attribs["viewBox"] = "0 0 " + " ".join(
            [str(x) for x in self.canvas_size]
        )
        ...
        for version_id, residue_validities in enumerate(
            self.data["residue_validities"]
        ):
            group_opacity = 1 if version_id == self.num_versions - 1 else 0
            shade_group = self.dwg.g(
                id=f"{self.svg_id}-shade-{version_id}", opacity=group_opacity
            )
                ...
                if not residue_valid:
                    shade_group.add(
                        self.dwg.polygon(
                            [
                                self.center,
                                self._coords_from_angle(
                                    self.angle_delta * segment_id, self.full_radius + 5
                                ),
                                self._coords_from_angle(
                                    self.angle_delta * (segment_id + 1),
                                    self.full_radius + 5,
                                ),
                            ],
                            stroke_opacity=0,
        ...
        for i in range(self.num_segments + 1):
            self.dwg.add(
                self.dwg.line(
                    self._coords_from_angle(
                        self.angle_delta * i, self.full_radius - 24
                    ),
        ...
        selector_points = (
            self._coords_from_angle(center_point, self.full_radius - 16),
            ...
            self.dwg.add(
                self.dwg.polygon(
                    [
                        ...
                        self._coords_from_angle(
                            self.angle_delta * (segment_id + 1), self.full_radius + 5
                        ),
                    ],
                    stroke=COLORS["BLACK"],
                    stroke_width=1,
    ...
    def _add_ring(self, ring_id, metric):
        ...
        self.dwg.add(
            self.dwg.polyline(
                [
                    self._coords_from_angle(
                        (CHAIN_VIEW_GAP_ANGLE / 25) * (i - (20 - 1) / 2),
                        ring_base_radius,
                        gap=False,
                    )
                    for i in range(20)
        ...
        if metric["type"] == "discrete":
            for version_id, version_datapoints in enumerate(datapoints):
                ...
                for segment_id, datapoint in enumerate(version_datapoints):
                    ...
                    segment_opacity = 1
                    if datapoint is not None and 0 <= datapoint < len(
                        metric["seq_colors"]
                    ):
                    ...
                    version_ring_segments.append(
                        (segment_points, segment_color, segment_opacity)
                    )
                group_opacity = 1 if version_id == self.num_versions - 1 else 0
                segment_group = self.dwg.g(
                    id=f"{self.svg_id}-discrete-{version_id}-{ring_id}",
            ...
            for version_deltas in deltas:
                version_magnitudes = [
                    x - avg_negative_delta if x is not None else None
                    for x in version_deltas
                ]
            ...
            magnitude_min, magnitude_max = (
                min(all_valid_magnitudes),
                max(all_valid_magnitudes),
            )
            ...
            for version_plot_magnitudes in plot_magnitudes:
                version_line_points = []
                zero_point = self._coords_from_angle(
                    self.angle_delta * 0.5, ring_base_radius
                )
            ...
            for point_id in range(baseline_point_resolution + 1):
                point_angle = (
                    (baseline_point_resolution - point_id)
                    * (2 * pi - CHAIN_VIEW_GAP_ANGLE)
                    / baseline_point_resolution
                )
                baseline_circle_points.append(
                    self._coords_from_angle(point_angle, ring_base_radius)
                )
            plot_points = line_points[-1] + baseline_circle_points
            ring_line = self.dwg.polyline(
                plot_points,
                stroke=metric["ring_color"],
                stroke_width=2,
                stroke_opacity=1,
            ...
            for version_id, version_line_points in enumerate(line_points):
                ...
                points_string = " ".join(
                    [",".join([str(x) for x in point]) for point in plot_points]
                )
                animation = Animate(
                    values=None,
                    dur="250ms",
