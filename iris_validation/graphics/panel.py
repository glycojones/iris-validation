    ...
    def _verify_chosen_metrics(self):
        ...
        for metric_list in (
            ...
            for metric_index in reversed(range(len(metric_list))):
                if (
                    metric_list[metric_index]["is_covariance"]
                    and not self.data[0]["has_covariance"]
                ):
                    del metric_list[metric_index]
                elif (
                    metric_list[metric_index]["is_molprobity"]
                    and not self.data[0]["has_molprobity"]
                ):
                    del metric_list[metric_index]
                elif (
                    metric_list[metric_index]["is_reflections"]
                    and not self.data[0]["has_reflections"]
                ):
                    del metric_list[metric_index]
                elif (
                    metric_list[metric_index]["is_rama_z"]
                    and not self.data[0]["has_rama_z"]
                ):
                    del metric_list[metric_index]
                elif (
                    metric_list[metric_index]["is_rama_classification"]
                    and self.data[0]["has_rama_z"]
                ):
                    del metric_list[metric_index]
    ...
    def _draw(self):
        ...
        self.dwg.add(
            self.dwg.text(
                text="Residue",
                insert=(
                    residue_view_bounds[0],
                    residue_view_bounds[1] + view_title_font,
                ),
                font_size=view_title_font,
                font_family="Arial",
        ...
        self.dwg.add(
            self.dwg.line(
                (chain_view_bounds[0], chain_view_bounds[1] + 40),
        ...
        self.dwg.add(
            self.dwg.line(
                (residue_view_bounds[0], residue_view_bounds[1] + 40),
        ...
        for chain_index, chain_id in enumerate(self.chain_ids[:12]):
            selector_color = (
                self.swtich_colors[1] if chain_index == 0 else self.swtich_colors[0]
            )
            self.dwg.add(
                self.dwg.rect(
                    insert=(
                        chain_view_bounds[0] + 75 + 50 * chain_index,
                        chain_view_bounds[1],
                    ),
            ...
            self.dwg.add(
                self.dwg.text(
                    text=chain_id,
                    insert=(
                        chain_view_bounds[0] + 75 + button_width / 2 + 50 * chain_index,
                        chain_view_bounds[1] + button_height / 2,
                    ),
                    font_size=view_title_font,
                    font_family="Arial",
                    text_anchor="middle",
        ...
        if len(self.chain_ids) > 12:
            ...
            self.dwg.add(
                self.dwg.text(
                    text="...",
                    insert=(
                        chain_view_bounds[0] + 85 + 50 * chain_index,
                        chain_view_bounds[1] + view_title_font,
                    ),
                    font_size=view_title_font,
                    font_family="Arial",
                )
        ...
        if self.num_models > 1:
            switch_group = self.dwg.g(
                id=f"{self.svg_id}-switch",
                onmouseover="setPointer();",
                onmouseout="unsetPointer();",
        ...
        switch_rectangle = self.dwg.rect(
            insert=(chain_view_bounds[2] - 140, chain_view_bounds[1]),
            size=(70, 30),
            rx=15,
            stroke_opacity=0,
            fill_opacity=1,
